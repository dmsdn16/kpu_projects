...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case StoC_MOVE_PLAYER: {
		StoC_move_player* move_packet = reinterpret_cast<StoC_move_player*>(packet);
			...
			if (ci == my_id) {
				if (0 != move_packet->move_time) {
					...
					if (global_delay < d_ms) global_delay++;
					   ...
					   break;
	case StoC_ADD_PLAYER: break;
	case StoC_REMOVE_PLAYER: break;
	case StoC_LOGIN_OK:
	{
		g_clients[ci].connected = true;
		active_clients++;
		StoC_login_ok* login_packet = reinterpret_cast<StoC_login_ok*>(packet);
		int my_id = ci;

...
void CGameObject::SetScaleX(float scale)
{
	m_xmf4x4World._11 = scale;
}

void CGameObject::SetScaleY(float scale)
{
	m_xmf4x4World._22 = scale;
}

void CGameObject::SetScaleZ(float scale)
{
	m_xmf4x4World._33 = scale;
}
...
void CUfoObject::Animate(float fTimeElapsed)
{
	switch (type) {
	case 1:
		Rotate(0.0f, 50.0f * fTimeElapsed, 0.0f);
		break;
	}
...
void CMissileObject::Animate(float fTimeElapsed) // 미사일 발사관리
{
	if (m_fire) {
